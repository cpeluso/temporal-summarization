"""
This file has the purpose of retrieving contextual data from
the 2013, 2014 and 2015 versions of the TREC Temporal Summarization datasets.

Two functions are exposed:
  * read_xml(path) has the purpose of retrieving the topics dataset, 
    which contains contextual information about each topic addressed 
    by the other datasets.

  * process_data(dfs) has the purpose of reformat the contextual information
    inside each dataset.
"""
...
def __query_id_to_topic_id(row):
...
def process_data(topics_df, matches_df, nuggets_df, updates_df):
  """
  Receives in input 4 DataFrames (topics, matches, nuggets, updates).
  Adds contextual information to matches, nuggets and updates DataFrames.
  """

  if "int" not in str(type(matches_df['query_id'][0])):
    matches_df['query_id'] = matches_df['query_id'].apply(str.strip)
    matches_df['topic_id'] = matches_df['query_id'].apply(__query_id_to_topic_id)
  ...
  if "int" not in str(type(updates_df['query_id'][0])):
    ...
    updates_df['topic_id'] = updates_df['query_id'].apply(__query_id_to_topic_id)
  ...
  if "int" not in str(type(nuggets_df['query_id'][0])):
    ...
    nuggets_df['topic_id'] = nuggets_df['query_id'].apply(__query_id_to_topic_id)
